...
- Вызов функции или метода может уже инкапсулировать test.step, cleanup и проверки. Не заявляй, что этого поведения нет, если реализация вызываемого кода не показана в diff. Отсутствие внешней обёртки вокруг вызова само по себе не является доказательством нарушения.
...
Ставь valid=false, если это предпочтение «на вырост», если кодекс допускает решение, если комментарий противоречит сам себе, путает порядок строк, игнорирует afterEach/finally, требует отсутствующий API или делает вывод из кода вне diff. В частности, вызов helper или метода нельзя объявлять нарушением из-за отсутствия внешнего test.step или cleanup, если реализация вызываемого кода не показана. Не применяй правила будущих уроков: если правила нет в переданном CODEX.md, комментарий невалиден. При сомнении — false. Каждый index верни ровно один раз.`,
        },
        {
          role: "user",
          content: `КОНТЕКСТ ДЗ
