...
type TestUser = {
  ...
  email: string;
  password: string;
  fullName: string;          // реальное имя (для поля «Имя» в профиле)
  timeZone: string;          // часовой пояс
  telegram: string;          // Telegram
  about: string;             // «О себе»
  skill: string;             // навык (то, что будем добавлять как «могу помочь»)
};
...
function makeUser(role: string, runId: number): TestUser {
  return {
    ...
    fullName: `${role} Полное Имя ${runId}`,
    timeZone: "Europe/Moscow",
    telegram: `@${role}_tg_${runId}`,
    about: `Это тестовый пользователь ${role}, создан в рамках автотеста (runId=${runId}).`,
    skill: `Playwright-demo-${runId}`,
  };
}
...
// Профиль: поля
const profileNameInput = (page: Page) => page.getByLabel("Имя");                 // или по test-id / id, если лейбл неоднозначен
const profileTimeZoneSelect = (page: Page) => page.locator("#pomidorqa-profile-timezone-select");
const profileTelegramInput = (page: Page) => page.locator("#pomidorqa-profile-telegram-input");
const profileAboutInput = (page: Page) => page.locator("#pomidorqa-profile-about-input");
...
test("основной путь + гонка за слот: регистрация → профиль (имя, часовой пояс, Telegram, о себе, навык) → слот → поиск в каталоге → бронирование → «Мои встречи» у обоих → второй гость видит ошибку", async ({
  browser,
}) => {
  ...
  await test.step("Хост: заполняет профиль (имя, часовой пояс, Telegram, о себе)", async () => {
    await hostPage.goto("/pomidorqa/profile");
    await profileNameInput(hostPage).fill(host.fullName);
    await profileTimeZoneSelect(hostPage).selectOption(host.timeZone);
    await profileTelegramInput(hostPage).fill(host.telegram);
    await profileAboutInput(hostPage).fill(host.about);
  });

  await test.step('Хост: добавляет навык «могу помочь» в профиле', async () => {
    await profileSkillInput(hostPage).fill(host.skill);
    await profileSkillTypeSelect(hostPage).selectOption("can_help");
    await profileSkillSubmit(hostPage).click();
    await expect(profileCanHelpSkills(hostPage)).toContainText(host.skill);
  });
  ...
  await test.step("Гость: ищет хоста в каталоге по навыку (сценарий 9)", async () => {
    await catalogFilterInput(guestPage).fill(guest.skill);
    await catalogFilterSubmit(guestPage).click();
    await expect(
      catalogCard(guestPage).filter({ hasText: host.fullName })
    ).toBeVisible();
  });

  await test.step("Гость: открывает карточку хоста", async () => {
    await catalogCard(guestPage).filter({ hasText: host.fullName }).click();
    await expect(personName(guestPage)).toHaveText(host.fullName);
  });
  ...
  await test.step("Гость2: регистрируется и тоже открывает окно бронирования на тот же слот", async () => {
    ...
    await catalogFilterInput(guest2Page).fill(guest2.skill);
    await catalogFilterSubmit(guest2Page).click();
    await catalogCard(guest2Page).filter({ hasText: host.fullName }).click();
    await expect(personName(guest2Page)).toHaveText(host.fullName);
  ...
  await test.step("Гость: видит бронирование в разделе «Мои встречи»", async () => {
    await expect(async () => {
      ...
      await expect(card).toHaveText(guest.fullName);
    }).toPass({ timeout: 10_000 });
  });
