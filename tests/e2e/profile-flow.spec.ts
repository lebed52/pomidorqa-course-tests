...
test.describe("Профиль: действия с полями", () => {
  ...
  test("навык: заполняем, выбираем тип и добавляем", async () => {
    ...
    await test.step("Добавляем навык «могу помочь»", async () => {
      ...
      await profilePage.profileSkillTypeSelect.selectOption("can_help");
      await profilePage.profileAddSkillButton.click();
    });

    await test.step("Навык появился в блоке «могу помочь»", async () => {
      await expect(profilePage.profileCanHelpSkills).toContainText(skillTag);
    });
  });
  ...
  test("негатив: пустой навык не добавляется", async () => {
    ...
    await test.step("Ни одного навыка не появилось", async () => {
      ...
      await expect(profilePage.profileSkillChips).toHaveCount(0);
      await expect(profilePage.profileCanHelpSkills).not.toBeVisible();
    });
  });
  ...
  test("негатив: навык «хочу разобрать» не попадает в блок «могу помочь»", async () => {
    ...
    await test.step("Добавляем навык «могу помочь»", async () => {
      await profilePage.profileSkillInput.fill(canHelpTag);
      await profilePage.profileSkillTypeSelect.selectOption("can_help");
      await profilePage.profileAddSkillButton.click();
      await expect(profilePage.profileSkillChips).toBeVisible();
    });

    await test.step("Добавляем навык «хочу разобрать»", async () => {
      await profilePage.profileSkillInput.fill(wantToLearnTag);
      await profilePage.profileSkillTypeSelect.selectOption("want_to_learn");
      await profilePage.profileAddSkillButton.click();
      await expect(profilePage.profileSkillChips).toBeVisible();
    });

    await test.step("Навыки разошлись по своим блокам", async () => {
      await expect(profilePage.profileSkillChips).toHaveCount(2);
      await expect(profilePage.profileCanHelpSkills).toContainText(canHelpTag);
      // Главная проверка теста: второй навык добавлен, но в «могу помочь» его нет.
      await expect(profilePage.profileCanHelpSkills).not.toContainText(wantToLearnTag);
    });
  });
