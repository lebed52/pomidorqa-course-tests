...
test.describe("Профиль: проверка полей", () => {
    ...
    test("Выбор случайной таймзоны из доступных", async ({page}) => {
        const allTimeZones = await profileTimeZoneSelect(page).locator("option").allTextContents();
        const randomTimeZone = allTimeZones[Math.floor(Math.random() * allTimeZones.length)];
        await profileTimeZoneSelect(page).selectOption(randomTimeZone);
        await clickButtonByName(page, "Сохранить");
        await expect(profileTimeZoneSelect(page)).toContainText(randomTimeZone);
    });
