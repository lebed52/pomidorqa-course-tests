...
test.describe("Профиль: карточка участника после регистрации", () => {
  ...
  test("добавление навыка «могу помочь»", async ({ page }) => {
    ...
    await test.step("Добавляем навык", async () => {
      ...
      await profileSkillTypeSelect(page).selectOption(SKILL_TYPE.canHelp);
  ...
  test("добавление навыка «хочу разобрать»", async ({ page }) => {
    const skillTag = `Playwright-hw9-want-${Date.now()}`;

    // Act
    await test.step("Добавляем навык с типом «хочу разобрать»", async () => {
      await profileSkillInput(page).fill(skillTag);
      await profileSkillTypeSelect(page).selectOption(SKILL_TYPE.wantToLearn);
      await profileAddSkillButton(page).click();
    });

    await test.step("Навык виден на странице, но не в блоке «могу помочь»", async () => {
      await expect(page.getByText(skillTag)).toBeVisible();

      const canHelpBlock = profileCanHelpSkills(page);
      if (await canHelpBlock.count() > 0) {
        await expect(canHelpBlock).not.toContainText(skillTag);
      }
    });
  });

  test("изменение нескольких полей профиля сразу сохраняется", async ({ page }) => {
    const newName = `Комбо Имя ${Date.now()}`;
    const about = `Комбо-описание ${Date.now()}`;

    // Act
    await test.step("Меняем имя и «О себе» одним сохранением", async () => {
      await profileNameInput(page).fill(newName);
      await profileAboutInput(page).fill(about);
      await saveProfileAndWaitResponse(page);
    });

    await test.step("После перезагрузки оба поля пришли с сервера", async () => {
      await page.reload();
      await expect(profileNameInput(page)).toHaveValue(newName);
      await expect(profileAboutInput(page)).toHaveValue(about);
    });
  ...
  test("незасохранённое изменение имени не остаётся после перезагрузки (негативный тест)", async ({
    page,
  }) => {
    const unsavedName = `Несохранённое имя ${Date.now()}`;

    // Act
    await test.step("Меняем имя, но не нажимаем «Сохранить»", async () => {
      await profileNameInput(page).fill(unsavedName);
    });

    await test.step("После перезагрузки страницы черновик не применился", async () => {
      await page.reload();
      await expect(profileNameInput(page)).not.toHaveValue(unsavedName);
    });
  });
});