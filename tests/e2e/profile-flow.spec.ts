...
test.describe("Тесты на изменение имени пользователя", () => {
  ...
  test('Добавление навыка "хочу разобрать"', async ({ page }) => {
    await test.step("Новый навык", async () => {
      ...
      await skillInput.fill(skillTag);
      await skillTypeSelect.selectOption("want_to_learn");
      await skillSubmitButton.click();
      await expect(skillAdded).toBeVisible();
    });
  });
  test('Добавление нескольких навыков "могу помочь"', async ({ page }) => {
    await test.step("Добавление нескольких навыков", async () => {
      const runId = Date.now();
      const skillTags = [`SQL-${runId}`, `Playwright-${runId}`];
      ...
      const addedSkills = page.locator(
        skillTags.map((tag) => `[data-skill-tag="${tag}"]`).join(", "),
      );
      await expect(addedSkills).toHaveCount(skillTags.length);
    });
  });

  test('Удаление навыка "хочу разобрать"', async ({ page }) => {
    await test.step("Удаление навыка", async () => {
      const skillTag = `SQL-${Date.now()}`;
      const { skillInput, skillTypeSelect, skillSubmitButton, skillAdded } =
        skillLocators(page, skillTag);
      await skillInput.fill(skillTag);
      ...
      await skillAdded.click();
      await expect(skillAdded).not.toBeVisible();
    });
  });
});
