...
// Урок 10: пользователь и регистрация — в helpers/user.ts,
// локаторы и действия профиля — в pages/profile-page.ts (class ProfilePage).
// В спеке остались только вызовы методов и expect — локаторов нет.
...
test.describe("Профиль: действия с полями", () => {
  ...
  test("навык: заполняем, выбираем тип и добавляем", async () => {
    ...
    // Комбо из трёх действий: ввод, выбор в списке, нажатие — спрятано в addSkill.
