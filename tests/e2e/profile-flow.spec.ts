...
const LOCATORS = {
  ...
  skill: {
    ...
    wantToLern: (page: Page) => page.locator('[data-skills="want_to_learn"]'),
    close: (page: Page) => page.locator('[title="Убрать"]'),
  },

  bugReportWidget: {
    button: (page: Page) => page.getByTestId("BugReportWidget-toggle"),
    panel: (page: Page) => page.getByTestId("BugReportWidget-panel"),
    bugButton: (page: Page) => page.getByTestId("BugReportWidget-type-bug"),
    ideaButton: (page: Page) => page.getByTestId("BugReportWidget-type-idea"),
    close: (page: Page) => page.getByTestId("BugReportWidget-close"),
  },
};
...
async function addSkill(page: Page) {
  ...
  await LOCATORS.skill.type(page).selectOption("want_to_learn");
...
test.describe("Профиль пользователя: редактирование полей", () => {
  ...
  test("При выпадающем меню 'Часового пояса' перекрывается блок 'О себе'", async ({
    page,
  }) => {
    await expect(LOCATORS.profile.bio(page)).toBeVisible();
    await LOCATORS.profile.timeZone(page).hover();
    await LOCATORS.profile.timeZone(page).click();
    await expect(LOCATORS.profile.bio(page)).toBeAttached();
  });

  test('Удаление навыка из блока "Могу помочь"', async ({ page }) => {
    const skillTag = `Skill-${Date.now()}`;
    await LOCATORS.skill.input(page).fill(skillTag);
    await LOCATORS.skill.type(page).selectOption("can_help");
    await LOCATORS.skill.add(page).click();
    await LOCATORS.skill.close(page).click();
    await expect(LOCATORS.skill.canHelp(page)).not.toBeVisible();
  });

  test('Добавление 3 навыков в блок "Хочу разобрать"', async ({ page }) => {
    await expect(LOCATORS.skill.wantToLern(page)).not.toBeVisible();
    await addSkill(page);
    await addSkill(page);
    await addSkill(page);
    await expect(LOCATORS.skill.close(page)).toHaveCount(3);
  });

  // Факультатив

  test('Открытие формы обратной связи при клике на кнопку "BugReport", повторное открытие формы для проверки состояния активности кнопки "Идея"', async ({
    page,
  }) => {
    await expect(LOCATORS.bugReportWidget.panel(page)).toBeHidden();
    await LOCATORS.bugReportWidget.button(page).click();
    await expect(LOCATORS.bugReportWidget.panel(page)).toBeVisible();
    await LOCATORS.bugReportWidget.ideaButton(page).click();
    await LOCATORS.bugReportWidget.close(page).click();
    await LOCATORS.bugReportWidget.button(page).click();
    await expect(LOCATORS.bugReportWidget.ideaButton(page)).toHaveClass(
      /text-white/,
    );
    await LOCATORS.bugReportWidget.close(page).click();
    await expect(LOCATORS.bugReportWidget.panel(page)).toBeHidden();
