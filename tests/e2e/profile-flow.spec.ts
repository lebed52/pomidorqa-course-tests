...
test("кнопка Сохранить отображается после изменения профиля", async ({ page }) => {
    await page.goto("/pomidorqa/profile");

    await page.getByLabel("О себе").fill("Тестовое описание");
...
test("поле Telegram пустое у нового пользователя", async ({ page }) => {
    await page.goto("/pomidorqa/profile");

    await expect(page.getByLabel("Telegram")).toHaveValue("");
});

test("навык SQL не отображается у нового пользователя", async ({ page }) => {
    await page.goto("/pomidorqa/profile");

    await expect(
        page.locator('[data-skill-tag="SQL"]')
    ).not.toBeVisible();
});