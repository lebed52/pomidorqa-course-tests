 ...
 * Статус локаторов:
 * ---------------------------------------------------------------------------
 * [ПОДТВЕРЖДЕНО] Регистрация, добавление навыка, добавление слота — те же
 *                голые локаторы, что в booking-flow.spec.ts, уже подтверждены
 *                зелёным прогоном на этих же страницах (/profile и /profile/slots).
 *
 * [ТРЕБУЕТ ВНИМАНИЯ] Смена имени — новый флоу: nameField грунтуется на
 *                    подтверждённом факте (лейбл "Имя" точно есть на сайте,
 *                    видели на регистрации), а не на позиции элемента в DOM.
 *                    saveButton и URL-паттерны в waitForResponse — обоснованные,
 *                    но непроверенные предположения. Сверить в DevTools перед PR.
 * ---------------------------------------------------------------------------
 *
