...
const nameInput = (page: Page) => page.locator('#pomidorqa-register-name');
const emailInput = (page: Page) => page.getByLabel("Email");
const passwordInput = (page: Page) => page.locator('#pomidorqa-register-password');
...
const dateInput =  (page: Page) => page.locator('input[name="date"][type="date"]');
const timeInput =  (page: Page) => page.locator('input[name="time"][type="time"]');
...
const confirmButton = (page: Page) => page.getByRole('dialog', {name: 'Подтвердить бронирование?'}).getByRole('button', { name: 'Подтвердить' });

const successMessage = (page: Page) => page.getByText("Забронировано! Договоритесь о ссылке на звонок — она в разделе «Мои встречи»");
const errorMessage = (page: Page) => page.getByRole('dialog').getByRole('alert');
...
test("основной путь + гонка за слот: регистрация → навык → слот → поиск в каталоге → бронирование → «Мои встречи» у обоих → второй гость видит ошибку", async ({
  ...
  await test.step("Гость: ищет хоста в каталоге по навыку (сценарий 9)", async () => {
    ...
    await expect(
      personCardByName(guestPage, host.name)).toBeVisible();
  });
  ...
  await test.step("Гость2: регистрируется и тоже открывает окно бронирования на тот же слот", async () => {
    ...
    await expect(personCardByName(guest2Page, host.name)).toBeVisible();
    ...
    await expect(async () => {
      const day2Chip = firstDate(guest2Page);
      if (!(await day2Chip.isVisible().catch(() => false))) {
        await guest2Page.reload();
      }
      await expect(day2Chip).toBeVisible();
    }).toPass({ timeout: 6_000 });
