...
test("вход с неверными данными — одинаковая ошибка в обоих случаях, без уточнения причины", async ({
  ...
  await test.step("Заводим реальный аккаунт для проверки", async () => {
    ...
    await page.locator("#pomidorqa-register-name").fill("Login Error Check");
    await page.locator("#pomidorqa-register-email").fill(email);
    await page.locator("#pomidorqa-register-password").fill(password);
  ...
  await test.step("Пробуем войти с верным email, но неверным паролем", async () => {
    ...
    await page.locator('label').filter({ hasText: 'Пароль' }).fill("wrong-password");
    await page.getByRole("button", {name: "Войти"}).click();
    const error = page.getByText('Неверный email или пароль');
  ...
  await test.step("Пробуем войти с несуществующим email", async () => {
    await page.goto("/pomidorqa/auth/login");
    await page.getByRole('textbox', { name: 'Email' }).fill(`no-such-user-${runId}@example.com`);
    await page.locator('label').filter({ hasText: 'Пароль' }).fill("any-password-123");
    await page.getByRole("button", {name: "Войти"}).click();
    const error = page.getByText('Неверный email или пароль');
    await expect(error).toBeVisible();
