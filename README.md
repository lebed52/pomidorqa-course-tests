...
- **Присылать pull request'ы** — начиная с Урока 5 (первый тренировочный PR) и особенно с Урока 16
  (полный путь тикет → тест → PR → зелёный CI)
...
Домашнее задание — дописать в `booking-flow.spec.ts` сценарий, где второй гость пытается
забронировать уже занятый слот хоста и должен увидеть ошибку. Подробности —
в описании эфира и чате марафона.

## Почему API-тест не бьёт по настоящей Supabase

У PomidorQA нет отдельного REST API — бронирование реализовано через Next.js Server Actions,
которые вызывают Postgres-функцию `pomidorqa_book_slot()` напрямую. `src/pyramid/mock-booking-api.ts`
поднимает локальный HTTP-сервер с той же бизнес-логикой (нельзя забронировать свой/чужой занятый/
прошедший слот, гонка на один слот разрешается ровно в одну подтверждённую бронь), чтобы показывать
API-уровень пирамиды без прод-секретов и без риска сетевой проблемы на эфире.

Официально API-тестирование и авторизация через `storageState` — вне рамок базового курса (см.
программу), этот уровень оставлен здесь как учебный пример пирамиды тестирования из Урока 2.
